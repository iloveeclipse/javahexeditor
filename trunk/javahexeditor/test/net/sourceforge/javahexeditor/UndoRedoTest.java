...
public class UndoRedoTest extends TestCase {
	...
	/*
	 * Test method for 'net.sourceforge.javahexeditor.BinaryContent.delete(long, long)'
	 */
	...
	public void testBlockDeleteAfterSingleInserts() {
		try {
			content.insert((byte)44, 127);  // 125, 126, 44, 127, 88, 77, 66, 128
			content.delete(126, 3);  // 125, 88, 77, 66, 128
			long[] result = content.undo();  // 125, 126, 44, 127, 88, 77, 66, 128
			assertEquals(126L, result[0]);
			assertEquals(129L, result[1]);
			assertEquals(8, content.get(data, 125));
			assertEquals(125, data.get(0));
	...
	public void testSigleDeletesToTheRightAfterSingleInserts() {
		try {
			content.insert((byte)44, 127);  // 124, 125, 126, 44, 127, 88, 77, 66, 128
			...
			long[] result = content.undo();  // 124, 125, 126, 44, 127, 88, 77, 66, 128
	...
	public void test2SingleInserts1SingleDelete() {
		try {
			content.insert((byte)44, 127);  // 125, 126, 44, 127, 88, 77, 66, 128
			content.insert((byte)45, 128);  // 125, 126, 44, 45, 127, 88, 77, 66, 128
			content.delete(128, 1);         // 125, 126, 44, 127, 88, 77, 66, 128
			long[] result = content.undo(); // 125, 126, 44, 45, 127, 88, 77, 66, 128
	...
	public void testBlockOverwrite() {
		try {
			content.overwrite(ByteBuffer.wrap(new byte[]{33, 44, 55}), 127L);  // 126, 33, 44, 55, 66
			content.overwrite(ByteBuffer.wrap(new byte[]{0, 1, 2}), 130L);  // 126, 33, 44, 55, 0, 1, 2
			assertTrue(content.canUndo());
			assertFalse(content.canRedo());
	...
	public void testNibbleInput() {
		try {
			content.overwrite((byte)0x0b, 0, 4, 127L);  // 126, 0x0bf, 88, 77, 66
			content.overwrite((byte)0x0c, 4, 4, 127L);  // 126, 0x0bc, 88, 77, 66
			assertTrue(content.canUndo());
			assertFalse(content.canRedo());
			assertEquals(8, content.get(data, 126L));
			assertEquals(126, data.get(0));
			...
			content.overwrite((byte)0x0e, 4, 4, 128L);  // 126, 0x0bc, 0x0de, 88, 77, 66
			assertTrue(content.canUndo());
			assertFalse(content.canRedo());
			data.position(0);
...
/*			assertEquals(2, rangesModified.size());
			assertEquals(128L, ((Long)rangesModified.get(0)).longValue());
			assertEquals(1L, ((Long)rangesModified.get(0)).longValue());
