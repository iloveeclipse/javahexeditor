...
final class DisplayedContent implements StyledTextContent {
	...
	myData.setLength(0);
	myData.append(text.substring(0,
		Math.min(text.length(), linesTimesColumns)));
	...
	event.newText = "";
	event.replaceCharCount = linesInText * numberOfColumns
		- linesTimesColumns + currentLimit;
	...
	if (forward) {
	    myData.delete(linesTimesColumns, linesTimesColumns
