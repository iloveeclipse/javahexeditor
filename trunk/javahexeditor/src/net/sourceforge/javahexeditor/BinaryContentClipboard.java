...
final class BinaryContentClipboard {
	...
	public Object nativeToJava(TransferData transferData) {
	    ...
	    File myData = null;
	    DataInputStream readIn = new DataInputStream(
	...
	while (!(success = file.delete())
		&& System.currentTimeMillis() - time < 1234L) {
	    System.gc();
	    try {
		Thread.sleep(333);
	...
	File lastPaste = (File) myClipboard.getContents(FileByteArrayTransfer
		.getInstance());
	myClipboard.dispose();
	...
	if (clipboardFile.canWrite() && clipboardFile.length() > 0L) {
	    try {
		RandomAccessFile file = new RandomAccessFile(clipboardFile,
			"rw");
		file.setLength(0L);
		file.close();
	    } catch (IOException e) {
	...
	for (int i = 0; i < available.length; ++i) {
	    if (MemoryByteArrayTransfer.getInstance().isSupportedType(
		    available[i])
		    || TextTransfer.getInstance().isSupportedType(available[i])
		    || FileByteArrayTransfer.getInstance().isSupportedType(
			    available[i])
     ...
     */
    private long tryGettingFileByteArray(BinaryContent content, long start,
	    boolean insert) {
	File lastPaste = (File) myClipboard.getContents(FileByteArrayTransfer
		.getInstance());
	if (lastPaste == null)
	...
	if (total > 0L) {
	    ...
	    Integer value = myFilesReferencesCounter.put(lastPaste,
		    new Integer(1));
	    if (value != null) {
		myFilesReferencesCounter.put(lastPaste,
    ...
    private long tryGettingFiles(BinaryContent content, long start,
	    boolean insert) {
	String[] files = (String[]) myClipboard.getContents(FileTransfer
		.getInstance());
	if (files == null)
	    return -1L;
    ...
    private long tryGettingMemoryByteArray(BinaryContent content, long start,
	    boolean insert) {
	byte[] byteArray = (byte[]) myClipboard
		.getContents(MemoryByteArrayTransfer.getInstance());
	if (byteArray == null) {
	    String text = (String) myClipboard.getContents(TextTransfer
		    .getInstance());
	    if (text != null)
