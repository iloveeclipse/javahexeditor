...
final class FindReplaceDialog extends Dialog {
	...
	public void widgetSelected(SelectionEvent e) {
	    if (lastIgnoreCase != ignoreCaseCheckBox.getSelection()
		    ...
		    || lastFindHexButtonSelected != findGroup.hexRadioButton
			    .getSelection()
		    || lastReplaceHexButtonSelected != replaceGroup.hexRadioButton
			    .getSelection()) {
	...
	public TextHexInputGroup(List<FindReplaceHistory.Entry> oldItems) {
		...
		throw new IllegalArgumentException(
			"Parameter 'oldItems' must not be null.");
	    }
	    items = oldItems;
	}
	...
	public void initialise() {
		...
		public void verifyText(VerifyEvent e) {
			...
			Matcher numberMatcher = patternHexDigits
				.matcher(e.text);
	...
	public void refreshCombo() {
	    ...
	    for (Iterator<FindReplaceHistory.Entry> iterator = items.iterator(); iterator
		    .hasNext();) {
	...
	public void rememberText() {
	    ...
	    items.add(
		    0,
		    new FindReplaceHistory.Entry(lastText, hexRadioButton
			    .getSelection()));
	    refreshCombo();
	}
	...
	if (selectionLength > 0L
	    ...
	    try {
		myTarget.myContent.get(ByteBuffer.wrap(selection),
			myTarget.getSelection().start);
	...
	feedbackLabel
		.setText(Texts.FIND_REPLACE_DIALOG_MESSAGE_SPECIFY_VALUE_TO_FIND);
	...
	if (findLiteral.length() > 0) {
		...
		message = TextUtility.format(
			...
			NumberUtility.getDecimalAndHexString(match
				.getStartPosition()));
	    } else {
		if (match.getException() == null) {
		    message = TextUtility.format(
			    Texts.FIND_REPLACE_DIALOG_MESSAGE_NOT_FOUND,
			    findLiteral);
		} else {
		    message = TextUtility
	...
	String message = TextUtility.format(
		Texts.FIND_REPLACE_DIALOG_MESSAGE_NOT_FOUND, findLiteral);
	...
	if (findLiteral.length() > 0) {
		...
		if (replacements == 1) {
		    message = TextUtility
			    .format(Texts.FIND_REPLACE_DIALOG_MESSAGE_ONE_REPLACEMENT,
				    findLiteral,
				    replaceLiteral,
				    NumberUtility
					    .getDecimalAndHexString(startPosition));
		    message = Texts.FIND_REPLACE_DIALOG_MESSAGE_ONE_REPLACEMENT;
		} else {
		    message = TextUtility
			    .format(Texts.FIND_REPLACE_DIALOG_MESSAGE_MANY_REPLACEMENTS,
				    NumberUtility
					    .getDecimalString(replacements),
				    findLiteral, replaceLiteral);
		}
	    } catch (IOException ex) {
		message = TextUtility
			.format(Texts.FIND_REPLACE_DIALOG_MESSAGE_ERROR_WHILE_REPLACING,
				findLiteral, replaceLiteral,
	...
	myTarget.replace(replaceGroup.textCombo.getText(),
		replaceGroup.hexRadioButton.getSelection());
    }
}
